...
async function createDirectChannel(channelId: string, sessionId: string, [initiatorAlias, initiatorSocket]: ChannelUser, [acceptorAlias, acceptorSocket]: ChannelUser, messageTimeoutMs = 20000): Promise<(() => void) | null> {
...
export default class SocketHandler {
    ...
    #mongoHandler: MongoUserHandler | null = null;
    ...
    constructor(username: string, sessionReference: string, ipRep: string, mongoHandler: MongoUserHandler, sessionCrypto: SessionCrypto) {
    ...
    private async registerNewSocket(ipRep: string, authToken: string, authData: string, socket: Socket) {
        ...
        for (let [event] of typedEntries(this.responseMap)) {
            const responseBy = this.responseMap[event].bind(this) as any;
    ...
    private async respond<E extends SocketClientSideEventsKey>(event: E, data: string, responseBy: (arg: SocketClientRequestParameters[E]) => Promise<SocketClientRequestReturn[E] | Failure>, resolve: (arg0: string | null) => void) {
